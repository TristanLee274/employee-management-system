# 🔒 Nhóm 3: Security & Distributed Systems

## Mục lục
1. [Spring Security & JWT](#1-spring-security--jwt)
2. [Microservices Architecture](#2-microservices-architecture)
3. [Kafka Event-Driven Communication](#3-kafka-event-driven)
...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http
            .csrf(AbstractHttpConfigurer::disable)  // Stateless → không cần CSRF
...
```java
@RestController
@RefreshScope  // Hot-reload config khi nhận POST /actuator/refresh
@RequiredArgsConstructor
...
> **Lưu ý Production:** Sử dụng Spring Cloud Bus + Kafka để broadcast refresh event đến tất cả instances cùng lúc thay vì gọi `/actuator/refresh` từng service.
