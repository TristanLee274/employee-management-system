...
class EmployeeServiceTest {
    ...
    private static final String JOB_TITLE = "Senior Developer";
    ...
    // @Nested: Groups related tests into an inner class for organizational clarity.
    // In test reports, these appear as a hierarchical tree:
    // EmployeeService — Unit Tests
    // └─ findById()
    // ├─ should return employee when found
    // └─ should throw EmployeeNotFoundException when not found
    class FindByIdTests {

        @Test
        @DisplayName("should return EmployeeResponse when employee exists")
        void should_ReturnEmployeeResponse_When_EmployeeExists() {
            // ── GIVEN (BDD: Arrange the preconditions) ────────────────────────
            // "Given an employee with ID 1 exists in the database"
            Employee employee = createTestEmployee();
            given(employeeRepository.findById(EMPLOYEE_ID))
                    .willReturn(Optional.of(employee));
            // BDDMockito.given() is the BDD-style equivalent of Mockito.when().
            // It reads naturally: "GIVEN that findById(1) is called, it WILL RETURN this
            // employee"

            // ── WHEN (BDD: Execute the action under test) ─────────────────────
            // "When findById is called with ID 1"
            EmployeeResponse result = employeeService.findById(EMPLOYEE_ID);

            // ── THEN (BDD: Assert the expected outcome) ───────────────────────
            // "Then the response should contain the employee's data"
            ...
            assertThat(result.salary()).isEqualByComparingTo(SALARY);
            ...
            then(employeeRepository).should(times(1)).findById(EMPLOYEE_ID);
        }
        ...
        void should_ThrowException_When_EmployeeNotFound() {
            // ── GIVEN ─────────────────────────────────────────────────────────
            // "Given no employee with ID 999 exists in the database"
            given(employeeRepository.findById(999L))
                    .willReturn(Optional.empty());

            // ── WHEN / THEN ───────────────────────────────────────────────────
            // "When findById is called with ID 999,
            // Then an EmployeeNotFoundException should be thrown"
            assertThatThrownBy(() -> employeeService.findById(999L))
                    .isInstanceOf(EmployeeNotFoundException.class);

            // Verify: repository was queried, but no further operations occurred
            then(employeeRepository).should(times(1)).findById(999L);
        }
    }
    ...
    class CreateTests {
        ...
        void should_CreateEmployeeAndPublishEvent_When_ValidRequest() {
            ...
            assertThat(capturedEmployee.getFirstName()).isEqualTo(FIRST_NAME);
            ...
            then(eventProducer).should().publishEvent(any());
        }
        ...
        void should_ThrowDuplicateEmailException_When_EmailExists() {
            ...
            then(eventProducer).should(never()).publishEvent(any());
        }
        ...
        void should_DefaultHireDateToToday_When_NotProvided() {
            ...
            then(employeeRepository).should().save(employeeCaptor.capture());
            Employee capturedEmployee = employeeCaptor.getValue();
            assertThat(capturedEmployee.getHireDate()).isEqualTo(LocalDate.now());
        }
    ...
    class UpdateTests {
        ...
        void should_UpdateOnlyProvidedFields_When_PartialUpdateRequest() {
            ...
            assertThat(existingEmployee.getFirstName()).isEqualTo(FIRST_NAME);
            assertThat(existingEmployee.getLastName()).isEqualTo(LAST_NAME);
            assertThat(existingEmployee.getEmail()).isEqualTo(EMAIL);
            assertThat(existingEmployee.getSalary()).isEqualByComparingTo(SALARY);
        }
        ...
        void should_ThrowDuplicateEmail_When_UpdatingToExistingEmail() {
            ...
            UpdateEmployeeRequest request = new UpdateEmployeeRequest(
                    null, null, "taken@company.com", null,
        ...
        void should_UpdateStatus_When_ValidStatusProvided() {
            ...
            UpdateEmployeeRequest request = new UpdateEmployeeRequest(
                    null, null, null, null, null, null, null, null, "ON_LEAVE");
        ...
        void should_ThrowException_When_InvalidStatusProvided() {
            ...
            UpdateEmployeeRequest request = new UpdateEmployeeRequest(
                    null, null, null, null, null, null, null, null, "INVALID_STATUS");
    ...
    class DeactivateTests {
        ...
        void should_SetStatusToInactive_When_EmployeeIsActive() {
            ...
            // ── THEN ──────────────────────────────────────────────────────────
            assertThat(activeEmployee.getStatus()).isEqualTo(EmployeeStatus.INACTIVE);
        }
    ...
    class SalaryCalculationTests {
        ...
        void should_CalculateSalary_When_ValidEmployeeAndType() {
            ...
            given(mockCalculator.calculate(SALARY, 160.0))
            ...
            then(mockCalculator).should().calculate(SALARY, 160.0);
        ...
        void should_ThrowException_When_EmployeeNotFoundForSalaryCalc() {
            ...
            // ── WHEN / THEN ───────────────────────────────────────────────────
            assertThatThrownBy(() -> employeeService.calculateSalary(999L, "MONTHLY", 0))
                    .isInstanceOf(EmployeeNotFoundException.class);
        ...
        void should_ThrowException_When_NoBaseSalaryConfigured() {
            ...
            employeeWithNoSalary.setSalary(null);
            given(employeeRepository.findById(EMPLOYEE_ID))
        ...
        void should_ThrowException_When_SalaryIsZero() {
            ...
            employeeWithZeroSalary.setSalary(BigDecimal.ZERO);
            given(employeeRepository.findById(EMPLOYEE_ID))
    ...
    class FindAllTests {
        ...
        void should_ReturnPaginatedList_When_ValidPaginationParams() {
            ...
            assertThat(result.getContent().getFirst().firstName()).isEqualTo(FIRST_NAME);
    ...
    class SearchTests {
        ...
        void should_ReturnMatchingEmployees_When_KeywordMatches() {
            ...
            given(employeeRepository.searchByKeyword(eq("John"), any(Pageable.class)))
                    .willReturn(page);
        ...
        void should_ReturnEmptyPage_When_NoMatchFound() {
            // ── GIVEN ─────────────────────────────────────────────────────────
            given(employeeRepository.searchByKeyword(eq("NonExistent"), any(Pageable.class)))
